...
export default function ShoppingCart() {
        ...
        <div className="mt-12 lg:grid lg:grid-cols-12 lg:items-start lg:gap-x-12 xl:gap-x-16">
          <section aria-labelledby="cart-heading" className="lg:col-span-7">
            <h2 id="cart-heading" className="sr-only">
              Artículos en su cesta de compra
            </h2>
          ...
          <section
            ...
            <dl className="mt-6 space-y-4">
              ...
              <dt className="flex items-center text-sm text-gray-600">
                <span>¿Tiene un código de cupón? </span>
              </dt>
              {/* errr */}
              {error && <ErrorMsg message={error?.message} />}
              {isAdded && (
                <SuccessMsg
                  message={`Felicidades usted ha obtenido ${coupon?.coupon?.discount} %`}
                />
              )}
              {/* success */}
              ...
              <form onSubmit={applyCouponSubmit}>
                <div className="mt-1">
                  <input
                    ...
                    placeholder="Ingrese un codigo de cupon"
                  />
                </div>
                {loading ? (
                  <LoadingComponent />
  ...
  {sumTotalPrice > 0 ? (
    ...
    <button
      disabled={true}
      className="w-full rounded-md border border-gray-300 bg-gray-200 py-3 px-4 text-base font-medium text-gray-400 cursor-not-allowed"
    >
      Proceder al pago
    </button>
  )}
